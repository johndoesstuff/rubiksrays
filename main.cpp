...
int main() {
	...
	while (true) {
		...
		Triangle current_tri = tri;
		glm::vec4 v0 = proj * view * glm::vec4(current_tri.points[0], 1.0f);
		glm::vec4 v1 = proj * view * glm::vec4(current_tri.points[1], 1.0f);
		glm::vec4 v2 = proj * view * glm::vec4(current_tri.points[2], 1.0f);
		...
		pixel0.foreground_color = current_tri.color;
		pixel0.character = U'#';
		pixel1.foreground_color = current_tri.color;
		pixel1.character = U'#';
		pixel2.foreground_color = current_tri.color;
		pixel2.character = U'#';
